...
def get_beetle_image(species_info, species):
    ...
    if species_info.get('photo_link'):
        try:
            print(
                f"Attempting to get image from species info: {species_info['photo_link']}")
            resp = requests.get(
                species_info['photo_link'], timeout=10, headers=headers)
        ...
        try:
            ...
            if infobox:
                ...
                if img and img.get("src"):
                    img_url = f"https:{img['src']}" if img['src'].startswith(
                        "//") else img['src']
                    print(f"Found image URL in Dutch infobox: {img_url}")
                    if '/thumb/' in img_url:
                        img_url = img_url.replace('/thumb/', '/')
                        img_url = img_url[:img_url.rfind(
                            '/') if '/' in img_url else None]
                    print(
                        f"Attempting to download infobox image from: {img_url}")
                    resp = requests.get(img_url, timeout=10, headers=headers)
                    print(
                        f"Dutch infobox image download status: {resp.status_code}")
                    if resp.status_code == 200:
                        beetle_image = BytesIO(resp.content)
                ...
                if img and img.get("src"):
                    ...
                    if '/thumb/' in img_url:
                        ...
                        img_url = img_url[:img_url.rfind(
                            '/') if '/' in img_url else None]
                    print(
                        f"Attempting to download content image from: {img_url}")
                    resp = requests.get(img_url, timeout=10, headers=headers)
                    print(
                        f"Dutch content image download status: {resp.status_code}")
                    if resp.status_code == 200:
                        beetle_image = BytesIO(resp.content)
            ...
            if not beetle_image:
                ...
                print(
                    f"Attempting to get image from English Wikipedia: {url_en}")
                ...
                if infobox:
                    ...
                    if img and img.get("src"):
                        ...
                        print(
                            f"Attempting to download infobox image from: {img_url}")
                        resp = requests.get(
                            img_url, timeout=10, headers=headers)
                        print(
                            f"English infobox image download status: {resp.status_code}")
                        if resp.status_code == 200:
                            beetle_image = BytesIO(resp.content)
                ...
                if not beetle_image:
                    ...
                    if img and img.get("src"):
                        ...
                        resp = requests.get(
                            img_url, timeout=10, headers=headers)
                        print(
                            f"English content image download status: {resp.status_code}")
                        if resp.status_code == 200:
                            beetle_image = BytesIO(resp.content)
...
def create_seasonal_graph(species):
    ...
    try:
        if client:
            ...
            """
            print(
                f"Attempting to get seasonal occurrence from LLM for {species}")
            response = client.chat.completions.create(
            ...
            try:
                ...
                if all(season in seasonal_data_raw and isinstance(seasonal_data_raw[season], (int, float)) for season in seasons_order):
                    seasonal_data = {season: max(0, float(seasonal_data_raw.get(
                        season, 0))) for season in seasons_order}  # Ensure non-negative floats
                    # Normalize percentages if total is not 100
                    total = sum(seasonal_data.values())
                    if total > 0:
                        seasonal_data = {
                            season: (count / total) * 100 for season, count in seasonal_data.items()}
                    print("Successfully parsed LLM seasonal data.")
                else:
                    print(
                        f"LLM returned unexpected JSON structure or values: {seasonal_data_raw}")
                    # Default if validation fails
                    seasonal_data = dict(zip(seasons_order, [25, 25, 25, 25]))

            except json.JSONDecodeError as json_e:
                print(f"Error decoding JSON from LLM seasonal data: {json_e}")
                # Default on JSON error
                seasonal_data = dict(zip(seasons_order, [25, 25, 25, 25]))
            except Exception as parse_e:
                print(
                    f"Error parsing LLM seasonal data after JSON decode: {parse_e}")
                # Default on other parsing error
                seasonal_data = dict(zip(seasons_order, [25, 25, 25, 25]))

        else:
            print("OpenAI client not initialized for seasonal data.")
            # Default if client fails
            seasonal_data = dict(zip(seasons_order, [25, 25, 25, 25]))

    except Exception as e:
        print(f"Error getting seasonal data from LLM: {e}")
        # Default on API error
        seasonal_data = dict(zip(seasons_order, [25, 25, 25, 25]))
    ...
    if counts:
        try:
            sns.barplot(x=seasons, y=counts, palette='viridis',
                        order=seasons_order)  # Ensure correct order
            ...
            plt.text(0.5, 0.5, 'Fout bij het maken van de grafiek',
                     ha='center', va='center')
    else:
        plt.text(0.5, 0.5, 'Geen data beschikbaar voor grafiek',
                 ha='center', va='center')
    ...
    try:
        ...
        try:
            if client and seasonal_data:
                ...
                seasonal_data_formatted = ", ".join(
                    [f"{season}: {percentage}%" for season, percentage in seasonal_data.items()])
                ...
                print(
                    f"Attempting to get seasonal analysis from LLM for {species} with data: {seasonal_data_formatted}")
                ...
                print(
                    "OpenAI client not initialized or no seasonal data for seasonal analysis.")
        except Exception as e:
            print(f"Error getting seasonal analysis from LLM: {e}")

        # Get rarity status
        rarity_status = get_rarity_status(
            species, species_info.get('description', ''))
        ...
        if species_info.get('latin_name'):
            elements.append(
                Paragraph(f"<i>{species_info['latin_name']}</i>", styles['Italic']))
        elements.append(Spacer(1, 12))

        # Rarity Status
        elements.append(
            Paragraph("<b>Zeldzaamheidstatus</b>", styles['Heading3']))
        ...
        if species_info.get('description'):
            elements.append(
                Paragraph("<b>Beschrijving</b>", styles['Heading3']))
            elements.append(
                Paragraph(species_info['description'], styles['Normal']))
            elements.append(Spacer(1, 12))

        # Locations
        if locations:
            elements.append(
                Paragraph("<b>Gevonden in</b>", styles['Heading3']))
        ...
        # Seasonality
        elements.append(
            Paragraph("<b>Seizoenspatroon</b>", styles['Heading3']))
        try:
        ...
        # Seasonal Analysis
        elements.append(
            Paragraph("<b>Seizoensanalyse</b>", styles['Heading3']))
        ...
        elements = [
            Paragraph(
                "Er is een fout opgetreden bij het genereren van het PDF rapport.", styles['Title']),
            Spacer(1, 12),
